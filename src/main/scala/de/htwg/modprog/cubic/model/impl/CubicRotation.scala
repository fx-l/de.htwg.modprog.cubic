...
import de.htwg.modprog.cubic.model.Rotation

class CubicRotation private(
    val players: List[Player], 
    val current: Player, 
    val waiting: List[Player]) extends Rotation {
  override def rotate = new CubicRotation(players, waiting.head, waiting.drop(1) :+ current)
  override def reset = new CubicRotation(players, players.head, players.tail)
  override def playerCount = players.length
}

object CubicRotation {
  def apply(players: Seq[Player]) = {
    require(players.length > 0)
    val namedPlayers = disambiguate(players.map(defaultName(_)).toList)
    new CubicRotation(namedPlayers, namedPlayers.head, namedPlayers.tail)
  }
